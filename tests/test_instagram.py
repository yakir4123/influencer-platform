...
def test_download_post_endpoint_success(client):
            ...
            "files": [
                {
                    "index": 0,
                    "name": "img1.jpg",
                    "path": "/absolute/path/to/downloads/social_downloads/instagram.com_p_C_abc123XYZ_abc123/img1.jpg",
                },
                {
                    "index": 1,
                    "name": "img2.jpg",
                    "path": "/absolute/path/to/downloads/social_downloads/instagram.com_p_C_abc123XYZ_abc123/img2.jpg",
                },
            ],
    ...
    with patch(
        "app.api.routes.download_instagram_post", return_value=mock_response
    ) as mock_download:
        payload = {
...
def test_download_post_endpoint_failure(client):
    with patch(
        "app.api.routes.download_instagram_post",
        side_effect=Exception("gallery-dl crashed"),
    ):
        payload = {
...
async def test_instagram_download_gcs_cache_hit(mock_settings, mock_list_gcs):
    ...
    assert (
        result["download_dir"]
        == "gs://test-bucket/instagram_posts/www.instagram.com_p_C_abc123XYZ_eadf2f7ab3"
    )
...
@patch("app.services.instagram.settings")
async def test_instagram_download_gcs_cache_miss(
    mock_settings, mock_gallery_dl, mock_list_gcs, mock_upload
):
    mock_settings.GCS_BUCKET_NAME = "test-bucket"
    mock_settings.INSTAGRAM_DOWNLOAD_TIMEOUT = 120
    mock_settings.INSTAGRAM_SAVE_TO = "temp"
    ...
    def fake_download_with_gallery_dl(
        url, out_dir, cookies_file, max_items, timeout, selected_image_index
    ):
    ...
    mock_upload.side_effect = (
        lambda local_file, gcs_path: f"gs://test-bucket/{gcs_path}"
    )
